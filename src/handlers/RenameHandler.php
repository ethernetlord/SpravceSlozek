<?php
class RenameHandler extends Handler {
  public function zpracuj($slozka) {
    try {
      $mf2d = new MultiFile2D();
      $files = $mf2d->ziskejNazvy2D(array(array("must_exist", $slozka), array("must_not_exist", $slozka)));
    ...
    foreach($files[0] as $i => $old_name) {
      $new_name = $files[1][$i];
      if(!rename($slozka . "/" . $old_name, $slozka . "/" . $new_name)) {
